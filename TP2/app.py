...
def get_text2img_pipe(model_id: str, scheduler_name: str):
    # Charger le pipeline text2img (caché)
...
prompt = st.text_area(
    "Prompt",
    value="ultra-realistic product photo of a leather sneaker on a white background, studio lighting, soft shadow, very sharp",
)
...
if run:
    ...
    if mode == "Text2Img":
        ...
        # Créer un pipeline img2img basé sur les mêmes composants
        pipe_i2i = to_img2img(pipe_t2i)

        # Normaliser l’image d’entrée à 512x512 (recommandé pour SD1.5)
        init_image_512 = ImageOps.fit(init_image, (512, 512), method=Image.Resampling.LANCZOS)
